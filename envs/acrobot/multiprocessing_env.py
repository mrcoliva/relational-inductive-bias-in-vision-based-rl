...
from envs.acrobot import service_pb2_grpc
from envs.acrobot.service_pb2 import StepRequest
...
class SimulatorVecEnv(DummyVecEnv):
    ...
    def __init__(self, env_fns, config, spaces=None):
        ...
        print(f'[SimulatorVecEnv] Communication via {self.communication_type}, port: {config["port_number"]}')

        self.ip_address = config['ip_address']
        self.start = 0
        self.nenvs = len(env_fns)
        self.n_envs = self.nenvs # for compatibility with stable_baselines
        ...
        if self.communication_type == 'ROS':
            ...
            self.socket.bind("tcp://*:" + str(self.port_number))
    ...
    def seed(self, seeds):
        assert len(seeds) == self.n_envs, 'Error: len(seeds) != self.n_envs'

        for i in range(len(seeds)):
            self.envs[i].seed(seeds[i])
    ...
    def _send_request(self, content):
        ...
        if self.communication_type == 'ROS':
            ...
            try:
                ...
                info = f'\n>>>> HINT: Sending request to environment failed. ' +\
                    'Have you started the simulator? See README.md for instructions.\n'
                raise grpc.RpcError(format_tb(e.__traceback__)[0] + info)
    ...
    def reset(self, should_reset=True):
        ...
        print(f'obs shape:  {np.array(observations_converted).shape}')
